...
class CommunicationThread extends Thread {
     ...
     * Thew report function will be used to signify when either a non - NTAG SmartSensor has been detected, or when at
     * least one page has been locked for writing by the PCD (Android phone).
     *
     * @return - @c true when communication can continue: the tag is an NTAG SmartSensor and all pages are available for
     * writing
     * - @c false when either the IC is not an NTAG SmartSensor, when some or all pages are locked for writing, or when
        ...
        public void handleMessage(Message message) {
            ...
            try {
                ...
                if (message.obj instanceof Tag) {
                    com.nxp.nhs31xx.demo.tlogger.Message.Message.hwVersion = com.nxp.nhs31xx.demo.tlogger.Message.Message.HwVersion.UNKNOWN; // reset
                    com.nxp.nhs31xx.demo.tlogger.Message.Message.swVersion = com.nxp.nhs31xx.demo.tlogger.Message.Message.SwVersion.UNKNOWN; // reset
                    tag = (Tag) (message.obj);
					ndef = Ndef.get(tag);
                    Log.e("part5", "Done");
                    if (BlockLockBits()) {
                        if (ndef == null) {
                            /* The tag is too late to provide an initial NDEF message in the NFC memory. */
                            report(R.string.noNdefFound_message);
                        } else {
                            List<Response> responses = writeReadNdef(null, true);
                            if ((responses == null) || (responses.size() == 0)) {
                                /* The tag seems to be too late to provide a meaningful initial NDEF
                                 * message in the NFC memory.
                                 * Try to kick-start it by sending a command, whereupon the tag will
                                 * hopefully reply with meaningful responses.
                                 * The byte sequence sent should trigger the expected full initial
                                 * NDEF message.
                                 */
